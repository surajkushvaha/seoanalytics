...
import MainImage from '../common/assets/images/login-image.webp';
  ...
  return (
    <div className="grid grid-cols-2 h-screen relative font-[rubik]">
      ...
      <div className='absolute'>
       <div className='grid grid-cols-2 w-[80vw] top-11 left-[10vw] h-[86vh] relative rounded-3xl shadow-md'>
       <div className='bg-white rounded-l-3xl'>
          <div className='flex justify-end w-full pe-10'>
            <ButtonComponent
             onClick={() => setActiveIndex(0)}
             label="Sign In"
             className={`px-6 py-2 z-20 rounded-b-md ${activeIndex === 0 ? 'bg-[#1f1f70] text-white' : ''}`}
            />
            <ButtonComponent
             onClick={() => setActiveIndex(1)}
             label="Register"
             className={`px-6 py-2 z-20 rounded-b-md ${activeIndex === 1 ? 'bg-[#1f1f70] text-white' : ''}`}
            />
          </div>
